...
import csv
from polynomial_agent_new import PolynomialAgent, PolynomialAgentBaseLine
import json
...
def game_loop(args):
    ...
    pygame.init()
    pygame.font.init()
    # It seems there was an issue with the previous approach. Let's debug and fix it.
    DATA_RECORD = True
    file_path_csv = './experiment_conditions.csv'
    df_condition_list = pd.read_csv(file_path_csv)

    SEARCHED = False
    while not SEARCHED:
        condition_num = random.randint(0, 26)
        model_selected = random.choice(df_condition_list.columns)
        if pd.isna(df_condition_list.iloc[condition_num][model_selected]):
            
            print('Selected :',model_selected, condition_num)
            SEARCHED = True
        else:
            print('Already Done',model_selected, condition_num)



    EGO_SPEED = ((condition_num//9)+4)*10.0
    DISTANCE = (((condition_num % 9)//3)+1)*10.0
    V_DIFF = (((condition_num % 9) % 3)+1)*10.0
    ...
    try:
        ...
        while True:
            ...
            if controller.EXPERIMENT_START:
                ...
                control_ego = EGO_agent.run_step(debug=False)
                ...
                data.append(
                    ...
                    EGO_agent._path[0][2][0] if EGO_agent._path else None, EGO_agent._path[0][2][3] if EGO_agent._path else None,
                    gap_btw_MV,
                    gap_btw_LV,
                    interation_probability,
                    EGO_agent.status,
                    round(controller.AccelPedal_FV,4) if controller.AccelPedal_FV else None , round(controller.BrakePedal_FV,4) if controller.BrakePedal_FV else None
                    ))
        ...
        if data and DATA_RECORD:
            df_condition_list.loc[condition_num, model_selected] = 'DONE'
            # SAVE CONDITION
            df_condition_list.to_csv(file_path_csv, index=False)
            base_path = "./DS_experiment_data/Validation Experiment_2/"
            base_name = model_selected+'_'+str(int(DISTANCE))+'_'+str(int(V_DIFF))+'_'+str(int(EGO_SPEED))+'_'+result+'.csv'
