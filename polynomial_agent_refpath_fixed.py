...
class PolynomialAgent(object):
    ...
    def run_step(self, debug = False):
        ...
        waypoint_distance = 500.0
        
        if self._cur_lane == None or (self._cur_lane != self._ego_wp.lane_id): # if lane is changed
            start_wp = self._ego_wp.previous(self._ego_wp.s)[0]
            self._cur_lane = self._ego_wp.lane_id
            self._waypoints_queue = self.getreferencewaypoints(self._unitlength,vehicle_speed_mps)
        else:
            add_num = self._search_idx(self._waypoints_queue)
            last_wp = self._waypoints_queue[-1]
            for i in range(add_num+1):
                self._waypoints_queue.append(last_wp.next(self._unitlength*(i+1)[0]))
        wpx, wpy = [], []
        for wp in self._waypoints_queue:
            wpx.append(wp.transform.location.x)
            wpy.append(wp.transform.location.y)
            ix = wp.transform.location.x
            iy = wp.transform.location.y
            self._debug.draw_point(carla.Location(x=ix,y=iy,z=0.3),0.05,carla.Color(0,0,0),0.1)
        self._csp = cubic_spline_planner.CubicSpline2D(wpx, wpy)
        # Generate Reference Path
        # s = np.arange(0, self._csp.s[-1], 1)


        ################################################## VEHICLE STATUS
        si_d, si_dd, di, di_d, di_dd = self._get_vehicle_status(self._csp)
        si = self._ego_wp.s
        ...
        # for i_s in s:
        #     ix, iy = self._csp.calc_position(i_s) 
        #     rx.append(ix)
        #     ry.append(iy)
        #     ryaw.append(self._csp.calc_yaw(i_s))
        #     rk.append(self._csp.calc_curvature(i_s))
        #     if self._isdebug:
        #         self._debug.draw_point(carla.Location(x=ix,y=iy,z=0.3),0.1,carla.Color(0,0,0),0.1)
        #         if i_s !=0:
        #             self._debug.draw_line(carla.Location(x=ix,y=iy,z=0.3),carla.Location(x=pix,y=piy,z=0.3),0.1,carla.Color(0,0,0),0.1)
        #         pix, piy = ix, iy
        ...
        path = self._frenet_optimal_planning(self._csp,si, si_d, si_dd, di, di_d, di_dd)
        if not path:
            control = carla.VehicleControl()
            control.throttle = 0.0
            control.steer = 0.0
...
############################################### 100m 앞까지의 waypoint 구하기, S 값
    def getreferencewaypoints(self, unitlength,vel_mps):
        wps = []
        remain = 100.0
        start_wp = self._ego_wp.next(max(0.5,vel_mps*self._dt))[0]
        end_wp = start_wp.next_until_lane_end(unitlength)[-1]
        while remain > end_wp.s-start_wp.s:
            print('end wp.s:',end_wp.s)
            print('start wp.s:',start_wp.s)
            wps.extend(start_wp.next_until_lane_end(unitlength))
            remain -= end_wp.s-self._ego_wp.s
            start_wp = end_wp.next(unitlength)[0]
            end_wp = start_wp.next_until_lane_end(unitlength)[-1]
            
        wps.append(start_wp)
        for i in range(int(remain/unitlength)):
            wps.append(start_wp.next((i+1)*unitlength)[0])

        return wps
    
    def _search_idx(self, wps):
        n = -1
        min_dis = float('inf')
        for wp in wps:
            if wp.transform.location.distance(self._ego_wp.transform.location) < min_dis :
                min_dis = wp.transform.location.distance(self._ego_wp.transform.location)
                n+=1
            else:
                break
        return n
